...
def plot_value_map(title, v, map_desc, color_map):
    ...
    for i in range(v.shape[0]):
        for j in range(v.shape[1]):
            ...
            red = v_red[i, j]
            value = np.round(np.float64(v[i, j]), 2)
            value = np.format_float_scientific(value, exp_digits=2, precision=2)
...
def plot_reward_and_delta_vs_steps(title, df, xlabel="Steps", ylabel="Reward"):
    ...
    ax.set_ylabel(ylabel, color="C1")

    lns1 = ax.plot(df.index.values, df['reward'], linewidth=1, label=ylabel, color="C1")

    ex_ax = ax.twinx()
    lns2 = ex_ax.plot(df.index.values, df['delta'], linewidth=1, label='Delta', color="C2")
    ex_ax.set_ylabel('Delta', color="C2")
    ex_ax.tick_params('y')
